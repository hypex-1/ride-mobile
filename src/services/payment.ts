...
class PaymentService {
  ...
  async logPayment(paymentData: {
    ...
    try {
      logger.log(' Logging payment:', paymentData);
      ...
      logger.log(' Payment logged successfully:', response);
      return response;
    } catch (error) {
  ...
  async getPaymentReceipt(rideId: string | number): Promise<PaymentReceipt> {
    try {
      logger.log(' Fetching payment receipt for ride:', rideId);

      const response = await apiService.get<PaymentReceipt>(`/payments/${rideId}`);
      
      logger.log(' Payment receipt fetched successfully');
      return response;
    } catch (error) {
  ...
  async processDigitalWalletPayment(paymentData: {
    ...
    try {
      ...
      logger.log(' Processing digital wallet payment (Coming Soon)');
      ...
      logger.error(' Error processing digital wallet payment:', error);
  ...
  async processCardPayment(paymentData: {
    ...
    try {
      ...
      logger.log(' Processing card payment (Coming Soon)');
